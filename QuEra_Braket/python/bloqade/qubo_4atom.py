...
'''
4-spin QUBO is subset of Ana's problem.
QUBO definition

E = 3*n0 + 6*n1  + 10*n2 +20*n3 + 8*n0n2 + 100*Exor(n0,n1)  + 100*Exor(n2,n3) 
...
def print_tiger2( L):
    print('     t0  t1  ')
    print('CN0:  %d   %d  '%(L[0],L[2]))
    print('CN1:  %d   %d  '%(L[1],L[3]))
    print('sequence: ',end='')
    for i in range(4):
...
if __name__=="__main__":
    ...
    atomLogId=[1,0,2,3]
    atomPos=[ (0,0), (4.9,0),(13.2,0), (18.1,0) ]
    localScale=[ 0.824, 1.0, 0.588, 0.0]  
    ...
    for tsweep in [3,6,10,30] :
        ...
        for key,val in counts.items():        
            m,e,_,_=val
            print('raw key=%s  mshot=%3d  ene=%7.1f (MHz)'%(key,m,e))
