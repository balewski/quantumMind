...
def main():
    ...
    parser.add_argument('-x','--xq', type=int, help='Qubit (0-8) to apply an X error on.')
    ...
    if len(parts) == 2:
        ...
        # Fallback if registers merged differently (should not happen with 2 regs)
        print("Warning: Unexpected syndrome format.")
    ...
    if x_corr is None and z_corr is None:
        ...
        # Check for Y error (X and Z on same qubit/block)
        x_type, x_loc = x_corr
        z_type, z_loc = z_corr # z_loc is representative qubit (0,3,6)
    ...
    if true_error_type == 'No error':
        ...
        if decoded_type == 'Z' and decoded_loc is not None:
            ...
            print(f"  (Verifying Z error in correct block: True Block={true_block}, Decoded Block={decoded_block})")
        else:
            passed = False
