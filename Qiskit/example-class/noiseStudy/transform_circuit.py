...
for seed in range(100,990):
    ...
    #q1=int(np.random.choice(range(1,19)))
    #if q1%5 not in [1,2,3] :continue
    #qubitLayout=[q1-1,q1,q1+1]
    qubitLayout=None #[18,15,16,17]
    #print(seed,'ql',qubitLayout)
    circOpt = transpile(circOrg, backend=backend, backend_properties=hw_properties, optimization_level=args.optimLevel, seed_transpiler=seed, initial_layout=qubitLayout)
    ...
    print(len(md5S),cisu['name'],cisu['operations'],' gates:',cisu['gates'],' measure:', meas_qubitL,' ancilla:',cisu['ancilla'] )#, 'md5:',md5[-10:])
