...
'''
  Hadamard-overlap Test for multi qubit state vector.
 Input:
...
#...!...!..................
def one_pair_test(ivecL,Uops): # ground truth + Hadamard-overlay test for a choice of Uops
    ...
    for op in Uops:  
        ...
        if verb>0: print('p0,p1:',p0,p1, 'p0-p1:',p0-p1)
        abABab.append(p0-p1)
        verb=0  # print only one time
    ...
    l2=np.linalg.norm(D)
    if abs(l2)<1e-4:
        print('CHECK Real PASSED  L2=%.3f  Uops=%s'%(l2,','.join(Uops)))
    else:
