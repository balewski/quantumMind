...
'''
Analyze  tomographpy of teleproting  experiment
- no graphics
'''
...
def ana_tomoTele(countsAll,md):
    ...
    for name,cntV in zip(nameL,countsAll):
        ...
        axMeas=name[-1]
        if 'tele' not in name: continue
        if 'tomo' not in name: continue
        n0=np.sum(cntV[0])
        n1=np.sum(cntV[1])        
        ns=n0+n1
        prob=n1/(n0+n1)
...
def verify_tomoU3(evD,md):
    ...
    print('verify:',msg,' shots=%d, nSigThr=%d, qubits: %s, exp=%s\n'%(shots,nSigThr,md['circ_sum']['text_qubits'],md['short_name']))
...
if __name__=="__main__":
    ...
    evD=ana_tomoTele(expD['counts_raw'],expMD)
    print('EV',evD)
    verify_tomoU3(evD,expMD)
    print('M:done')
