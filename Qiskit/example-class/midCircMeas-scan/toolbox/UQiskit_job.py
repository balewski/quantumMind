...
def submit_args_parser(backName,parser=None):
    ...
    parser.add_argument('-q','--qubits', default=[0], nargs='+', type=int, help='physical qubit ids, space separated')
...
def harvest_submitInfo(job,md,taskName='exp'):
    ...
    sd['tot_qubits']=backend.configuration().num_qubits
...
def postprocess_job_results(rawCountsL,md,expD,backend=None):
    ...
    for ic in range(csd['num_circ']):
        if csd['num_mid_clbits']>0 :  # pairs of measurements
            cntV=unpack_qiskit_midCM_counts(rawCountsL[ic],csd['num_clbits'],csd['num_mid_clbits'])
        else:
...
#...!...!....................
def unpack_qiskit_midCM_counts(qk_counts,post_clbits,pre_clbits,toBigEndian=False):
    # convert qiskit.result.counts.Counts to numpy [ncirc, prem,postm]
    # Qiskit is using LSB order, aka small-endian

    preNB=1<<pre_clbits  # number of possible bitstrings
    postNB=1<<post_clbits
    counts=np.zeros((postNB,preNB),dtype=np.int32)
    ...
    postS,preS=key0.split(' ')
    #print('aa',preS,postS)
    assert len(preS)==pre_clbits
    ...
    for key in keyL:
        postS,preS=key.split(' ')
        preA=BitArray(bin=preS)
        postA=BitArray(bin=postS)
        counts[postA.uint,preA.uint]=qk_counts[key]  # retain Qiksit order
...
#...!...!....................
def dump_qasm3(circuit, backend):
    import qiskit.qasm3
    return qiskit.qasm3.Exporter(
