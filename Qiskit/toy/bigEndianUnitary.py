...
def print_complex_nice_matrix(array, tol=1e-10):
    ...
    formatted_array = np.vectorize(complex_formatter)(array)

    # Print array row by row
    for row in formatted_array:
        print('  '.join(row))

def print_complex_nice_vector(vector, tol=1e-10):
    # Set print options for better readability
    np.set_printoptions(precision=2, suppress=True)
    ...
    formatted_vector = np.vectorize(complex_formatter)(vector)
