...
using Qiskit. It does not rely on the high-level `qiskit_experiments` framework, instead
building all components manually to demonstrate the underlying process.
...
2.  **Construct RB Sequences:** For a range of sequence lengths 'm', it generates multiple
    random sequences of Clifford gates. For each sequence, it classically calculates the
    single inverse gate that returns the qubit to its initial state.
...
4.  **Transpile and Execute:** The generated circuits are transpiled for the specific
    backend and target qubit, ensuring they are decomposed into the backend's native
    basis gates. The job is then run on the simulator.
5.  **Analyze and Fit Results:** All raw data is processed. The average survival
    ...
    Errors on the fit parameters, the Error Per Clifford (EPC), and the reduced
    chi-squared are all calculated in this step.
6.  **Print Summary:** A consolidated summary of all calculated results from the
    analysis is printed to the terminal.
7.  **Plot Results:** The final results are visualized. The plot includes:
    - Violin plots showing the full distribution of survival probabilities for each 'm'.
    - A scatter plot of the mean survival probabilities.
    - The fitted exponential decay curve.
    - A detailed legend with the fitted parameters, their errors, the EPC, and the
...
def XXgenerate_single_qubit_clifford_group():
    """
    Generates the 24 single-qubit Clifford gates using the group generators H and S.
    This method is robust and guarantees the generation of the complete group.
    """
    ...
    while queue:
        current_cliff = queue.pop(0)
        
        generators = [cliff_h, cliff_s]
        for gen in generators:
            new_cliff = current_cliff.compose(gen)
            
            is_new = True
            for existing_cliff in unique_cliffords:
                # Use the == operator for comparison
                if new_cliff == existing_cliff:
                    is_new = False
                    break
            
            if is_new:
                unique_cliffords.append(new_cliff)
                queue.append(new_cliff)

    if len(unique_cliffords) != 24:
        print(f"Warning: Generated {len(unique_cliffords)} Cliffords, expected 24. There is an issue in the generation logic.")

    return unique_cliffords
...
#print('M: single_qubit_cliffords:',single_qubit_cliffords)
...
for m in lengths:
    ...
    for i in range(num_samples):
        ...
        for _ in range(m):
            ...
            if i==0: print( rand_cliff_idx,cliffords_names[rand_cliff_idx], ', ',end='')
            qc.append(random_clifford.to_instruction(), [0])
            qc.barrier()
            tracking_clifford = tracking_clifford.compose(random_clifford)
        inverse_clifford = tracking_clifford.adjoint()
        qc.append(inverse_clifford.to_instruction(), [0])
        qc.measure(0, 0)
        qc.metadata = {'xval': m}
...
ax.set_title(f"Single-Qubit custom RB  on {backend.name} (Qubit {qubit_to_test})", fontsize=14)
ax.grid(True, which='both', linestyle='--', linewidth=0.5)
ax.legend(fontsize=10)
#ax.set_ylim(bottom=min(0.4, np.min(avg_probs) - 0.1))

# Save the plot
output_dir = 'out'
os.makedirs(output_dir, exist_ok=True)
outF = os.path.join(output_dir, f'rbB_{backend.name}_q{qubit_to_test}.png')
